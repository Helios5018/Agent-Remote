...
Agent 详情页默认不显示文件页签。在右上角「⋯」选择「打开文件系统」后，在顶栏出现「会话 / 文件」切换并进入文件页。文件页「⋯」或详情页「⋯」都可以关闭模块，关闭不会删除文件。是否打开按 Agent 在当前浏览器中记忆；目录标签、选中项、滚动位置和复制队列在各 Agent 之间共享。
...
三列鼠标交互：中列点击只切换当前项；右列点击前进一级并选中该项，文件自动预览；左列点击返回上一级。窄屏保留单列点击进入。电脑上的快捷键在文件区获得焦点时生效（单列也支持），不接管 Agent、输入框、播放器或确认弹窗。
