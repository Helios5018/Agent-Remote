...
interface BrowserState { tabs: BrowserTab[]; active: string; clipboard: string[]; clipboardMode?: "copy" | "move" }
const KEY = "car.files.v2";
function initial(): BrowserState {
  try { const saved = localStorage.getItem(KEY); const s = JSON.parse(saved ?? localStorage.getItem("car.files.v1") ?? "null"); if (s && Array.isArray(s.tabs) && s.tabs.every((t: BrowserTab) => typeof t.path === "string" && Array.isArray(t.selected) && typeof t.query === "string") && s.tabs.some((t: BrowserTab) => t.id === s.active)) return { ...s, tabs: s.tabs.map((t: BrowserTab) => ({ ...t, hidden: saved ? (t.hidden ?? true) : true })), clipboard: Array.isArray(s.clipboard) ? s.clipboard : [] }; } catch { /* unavailable storage */ }
...
export function addTab(path: string) { const id = crypto.randomUUID(); setFiles({ ...state, active: id, tabs: [...state.tabs, { id, path, scroll: 0, selected: [], query: "", mode: "filter", hidden: true, loaded: 100 }] }); }
